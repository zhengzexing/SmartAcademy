...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    private String getMimeType(String extension) {
        ...
        //通用mimeType字节流，设置通用的值作为默认值，防止后缀extension == null时，没有文件的类型
     ...
     * @param bucket              上传文件存放的桶 testBucket
     * @param objectName          上传文件的存放的层级目录以及对象名称 /001/1.mp4
    ...
    public MediaFiles addMediaFilesToDb(Long companyId, String fileMd5, UploadFileParamsDto uploadFileParamsDto, String bucket, String objectName) {
        ...
        // 没有该文件则将文件信息插入到文件表中，若有则将文件信息修改到文件表中
        if (mediaFiles == null) {
    ...
    private void addWaitingTask(MediaFiles mediaFiles){
        ...
        if(mimeType.equals("video/x-matroska")){
            ...
            mediaProcess.setCreateDate(LocalDateTime.now());//创建时间
    ...
    /**
     * 将文件上传到minio中，同时将文件信息保存到数据库文件表中
    ...
    public RestResponse uploadChunk(String fileMd5, int chunk, String localFilePath) {
        ...
        try {
            //将文件存储至minIO，分块文件没有后缀，使用getMimeType("")获取类型
            addMediaFilesToMinIO(localFilePath, getMimeType(""), bucket_videoFiles, chunkFilePath);
            return RestResponse.success(true);
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public File downloadFileFromMinIO(String bucket,String objectName){
        ...
        try{
            InputStream stream = minioClient.getObject(GetObjectArgs.builder()
                    ...
                    .build());
            //创建临时文件
            minioFile=File.createTempFile("minio", ".merge", new File("D:\\Java\\Java源代码\\学成在线\\TempFiles"));
